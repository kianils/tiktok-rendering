...
import type { ArchiveEvent, ParsedArchive, RenderPrimitive } from "./types";
...
function pickDate(obj: Record<string, unknown>): string | null {
  for (const [k, v] of Object.entries(obj)) {
    if (!/date|time|timestamp|created/i.test(k)) continue;
...
function primitiveFromPath(path: string): RenderPrimitive {
  const p = path.toLowerCase();
  if (p.includes("search")) return "intent";
  if (p.includes("like") || p.includes("favorite") || p.includes("follow")) return "preference";
  if (p.includes("comment") || p.includes("message") || p.includes("chat")) return "social";
  if (p.includes("login") || p.includes("device") || p.includes("setting")) return "account";
  if (
    p.includes("activity") ||
    p.includes("browse") ||
    p.includes("watch") ||
...
function labelFromRecord(obj: Record<string, unknown>): string {
  const parts: string[] = [];
  for (const [k, v] of Object.entries(obj)) {
    if (parts.length >= 4) break;
    if (typeof v === "string" && v.trim()) parts.push(`${k}: ${truncate(v, 80)}`);
...
function previewRecord(obj: Record<string, unknown>): string {
  try {
  ...
  if (Array.isArray(data)) {
    data.forEach((item, i) => {
      ...
      if (item && typeof item === "object" && !Array.isArray(item)) {
        const ev = objectToEvent(item as Record<string, unknown>, sourceFile, `${jsonPath}[${i}]`);
        if (ev) out.push(ev);
...
function parseActivityTxt(content: string, sourceFile: string, out: ArchiveEvent[]): void {
...
async function collectPointerEvents(zip: JSZip, root: unknown, warnings: string[]): Promise<ArchiveEvent[]> {
  ...
  for (const v of Object.values(root as Record<string, unknown>)) {
    ...
    try {
      ...
      warnings.push(`Failed to parse referenced file ${p}: ${(e as Error).message}`);
    }
    if (out.length >= MAX_EVENTS) break;
  }
  return out;
...
export async function parseTikTokExportZip(file: File): Promise<ParsedArchive> {
  idCounter = 0;
  ...
  for (const meta of jsonFiles) {
    ...
    try {
      ...
      warnings.push(`JSON parse failed for ${meta.path}: ${(e as Error).message}`);
    }
  }

  const orphanTxt = inventory.filter(
    (i) => i.path.toLowerCase().endsWith(".txt") && !events.some((e) => e.sourceFile === i.path),
  );
  for (const meta of orphanTxt) {
    ...
    try {
      ...
      warnings.push(`TXT read failed for ${meta.path}: ${(e as Error).message}`);
    }
  }
