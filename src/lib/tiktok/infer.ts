...
export const INFERENCE_MODEL_VERSION = "content-first-v2";
export const FEATURE_EXTRACTION_RULESET_VERSION = "regex-ruleset-v1";
 ...
 */
export const COUNTERFACTUAL_BORDERLINE_RADIUS = 4;
...
function textFor(ev: ArchiveEvent): string {
 ...
 */
function extractFeatures(ev: ArchiveEvent): Features {
  ...
  return {
    ...
    hasExplicitAction:
      /\b(like|liked|favorite|save|saved|share|shared|follow|followed|comment|react)\b/.test(text),
    // Consumption/attention: watch-time-adjacent signals even when dwell time isn't exported.
    hasWatchSignals:
      /\b(watch|watched|view|viewed|duration|dwell|rewatch|paused|skip|completion|play)\b/.test(text),
    ...
    hasIdentitySignals:
      /\b(account|profile|login|signin|sign in|session|user id|userid|uid|cookie|token)\b/.test(text),
...
function toConfidence(score: number): Confidence {
  if (score >= CONFIDENCE_CUTOFFS.high) return "high";
  if (score >= CONFIDENCE_CUTOFFS.medium) return "medium";
  return "low";
}

function scoreFeatures(features: Features, ev: ArchiveEvent) {
  ...
  const primitivePrior =
    ev.primitive === "unknown" ? 0 : ev.primitive === "preference" || ev.primitive === "intent" ? 3 : 1;

  const dataQualityScore = clamp100(
    (features.hasTimestamp ? 40 : 0) + Math.min(30, features.tokenCount / 3) + (features.hasUrl ? 10 : 0),
  );

  const signalRichnessScore = clamp100(
    (features.hasExplicitAction ? 18 : 0) +
  ...
  const identityLinkageScore = clamp100(
    (features.hasIdentitySignals ? 40 : 0) +
      (features.hasDeviceSignals ? 25 : 0) +
      (features.hasLocationSignals ? 20 : 0) +
      (features.hasTimestamp ? 15 : 0),
  );

  const surplusScore = clamp100(
    SURPLUS_WEIGHTS.signalRichness * signalRichnessScore +
      SURPLUS_WEIGHTS.identityLinkage * identityLinkageScore +
      SURPLUS_WEIGHTS.dataQuality * dataQualityScore +
      (features.hasWatchSignals ? SURPLUS_WEIGHTS.watchBonus : 0) +
      (features.hasSearchSignals ? SURPLUS_WEIGHTS.searchBonus : 0),
  );

  return { dataQualityScore, signalRichnessScore, identityLinkageScore, surplusScore };
}

function signalsFromFeatures(features: Features): SignalType[] {
  const out: SignalType[] = [];
  if (features.hasExplicitAction) out.push("explicit_feedback_actions");
  if (features.hasWatchSignals) out.push("watch_time_or_consumption_signals");
  if (features.hasSearchSignals) out.push("text_query_intent_signals");
  if (features.hasSocialSignals) out.push("social_graph_interaction_signals");
  if (features.hasIdentitySignals) out.push("account_or_session_linkage_signals");
  if (features.hasDeviceSignals) out.push("device_fingerprint_signals");
  if (features.hasLocationSignals) out.push("location_or_locale_signals");
  if (features.hasInterestSignals) out.push("topic_or_hashtag_signals");
  if (!out.length) out.push("low_structured_signal_content");
  return out;
}

function surplusVerdict(score: number): SurplusVerdict {
  if (score >= SURPLUS_THRESHOLDS.likelySurplusMin) return "likely_surplus";
  if (score >= SURPLUS_THRESHOLDS.mixedMin) return "mixed";
  return "unclear";
}
 ...
 */
function uncertaintyFromMetrics(metrics: RowInference["metrics"]): InferenceUncertainty {
  const distanceToBoundary = Math.min(
    Math.abs(metrics.surplusScore - SURPLUS_THRESHOLDS.likelySurplusMin),
    Math.abs(metrics.surplusScore - SURPLUS_THRESHOLDS.mixedMin),
  );
  return {
    ...
    counterfactualSensitivity:
      distanceToBoundary <= COUNTERFACTUAL_BORDERLINE_RADIUS ? "borderline" : "stable",
  };
}

function renderingFromSignals(signals: string[]): string {
...
function surplusRationale(score: number, metrics: RowInference["metrics"]): string {
...
function loopFromSignals(signals: string[]): string {
...
export function inferRow(ev: ArchiveEvent): RowInference {
...
export function surplusCounts(events: ArchiveEvent[]): Record<SurplusVerdict, number> {
