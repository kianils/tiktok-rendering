...
exports.TOP_CASCADES = exports.LOOPS_MODEL_VERSION = void 0;
exports.computeLoops = computeLoops;
...
function isSearchEvent(ev) {
    return ev.sourceFile.toLowerCase().includes("searches.txt");
}
function isWatchEvent(ev) {
    const f = ev.sourceFile.toLowerCase();
    return f.includes("watch history") || f.includes("watch_history");
}
...
function computeLoops(events, patterns) {
    const dated = events
        .filter((e) => Boolean(e.at))
        .sort((a, b) => (a.at < b.at ? -1 : 1));
    const GAP_MS = patterns_1.SESSION_GAP_MINUTES * 60 * 1000;
    ...
    for (let i = 0; i < dated.length; i += 1) {
        ...
        for (let j = i + 1; j < dated.length; j += 1) {
            ...
            if (nextMs - lastMs > GAP_MS)
                break; // session boundary
            if (isWatchEvent(next))
                followUps += 1;
            lastMs = nextMs;
        }
    ...
    const meanFollowUpWatchesPerSearch = searchesAnalyzed > 0 ? Number((totalFollowUpWatches / searchesAnalyzed).toFixed(2)) : 0;
    const medianFollowUpWatchesPerSearch = Number(medianOf(cascades.map((c) => c.followUpWatchCount)).toFixed(2));
    const topCascades = [...cascades]
        .sort((a, b) => b.followUpWatchCount - a.followUpWatchCount)
        .slice(0, exports.TOP_CASCADES);
    const cascadePlain = searchesAnalyzed === 0
    ...
    const meanGapHours = gapsHours.length > 0
    ...
    const fastReturnPct = gapsHours.length > 0 ? Math.round((fastReturnCount / gapsHours.length) * 100) : 0;
    const cadencePlain = gapsHours.length === 0
    ...
    const returningInterestRate = uniqueTermsSearched > 0
    ...
    const returningPlain = uniqueTermsSearched === 0
    ...
    const evidenceBasis = `Search→watch cascade: ${searchesAnalyzed} searches analyzed, ${totalFollowUpWatches} total follow-up ` +
    ...
    const claimBoundary = "Describes loops visible in your disclosed behavior trace only. Does not claim that TikTok " +
        "specifically used any one row to reinforce any one interest, and does not infer emotional state.";
    return {
        modelVersion: exports.LOOPS_MODEL_VERSION,
